...
const formatCurrency = (value) => `EUR ${Number(value || 0).toLocaleString("en-US", { maximumFractionDigits: 2 })}`;
...
const Dashboard = () => {
  ...
  const alerts = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        description: lastEventText ? `Last CTT webhook event (${statusLabel || "Updated"}) at ${lastEventText}.` : "No CTT webhook events received yet.",
        tone: "info"
      }
    ];
  }, [cttWebhook]);
      ...
      <div className="grid gap-4 lg:grid-cols-[1.3fr_1fr]">
        <Card className="border-border/60 bg-card/90">
          ...
          <CardContent className="space-y-4">
            {dashboardLoading ? <p className="text-sm text-muted-foreground">Loading live store distribution...</p> : storeOrders.length === 0 ? <p className="text-sm text-muted-foreground">No routed orders available yet.</p> : storeOrders.map((store) => <div key={store.name} className="space-y-2">
        ...
        <Card className="border-border/60 bg-card/90">
          ...
          <CardContent className="space-y-3">
            {dashboardLoading ? <p className="text-sm text-muted-foreground">Loading low stock products...</p> : lowStockProducts.length === 0 ? <p className="text-sm text-muted-foreground">No products are below 10 units.</p> : visibleLowStockProducts.map((item) => <div
  ...
  >
            ...
            {!dashboardLoading && lowStockProducts.length > 4 ? <Button
    variant="outline"
    size="sm"
  ...
  >
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
        <Card className="border-border/60 bg-card/90">
          ...
          <CardContent className="space-y-3">
            {alerts.map((alert) => <Alert key={alert.title} className="bg-background/70">
                <AlertTriangle className="h-4 w-4" />
